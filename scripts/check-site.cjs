...
async function main(){
  ...
  try{
    ...
    for(const width of [1440,768,390,320]){
      ...
      assert.match(await page.title(),/^FoodLoop/);
      ...
      assert(links.length>=3&&links.every(x=>x==='https://t.me/plan_eat_ai_bot'),'Bot links changed or missing');
      const data=await page.locator('#jsonld').textContent();assert(JSON.parse(data)['@graph'].some(x=>x.name==='FoodLoop у Telegram'));
      ...
      report.push(`PASS ${width}px: no overflow, FoodLoop metadata, 4 tabs, keyboard, FAQ, Telegram links, all images loaded`);
    }
