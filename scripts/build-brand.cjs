/* FoodLoop vector identity. Run with Node + sharp; no image-generation API needed. */
...
async function main() {
  fs.writeFileSync(path.join(dir,'foodloop-mark.svg'), svg(mark()));
  ...
  for(const [name,fg] of [['dark',colors.pine],['light',colors.cream]]) {
    const lockup=svg(`<g transform="translate(0 1) scale(.88)">${mark(fg)}</g><text x="86" y="58" font-family="Arial, sans-serif" font-size="53" font-weight="700" letter-spacing="-2.8" fill="${fg}">FoodLoop</text>`,340,80);
    fs.writeFileSync(path.join(dir,`foodloop-logo-${name}.svg`),lockup);
    await sharp(Buffer.from(lockup)).resize(1360,320).png().toFile(path.join(dir,`foodloop-logo-${name}.png`));
  }
  const symbol=[];
